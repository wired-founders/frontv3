...
export default function CampaignPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const data = await fetchAnalytics("ad_account", activeAccountId);
        setCampaigns(data || []);
      } finally {
        setLoading(false);
      }
  ...
  if (!activeAccountId) {
    return <div className="p-4 text-sm text-gray-500">Connect an ad account to view campaigns.</div>;
  }
            ...
            {campaigns.map((camp) => (
              <div key={camp.id} className="p-4 border rounded-md bg-white dark:bg-neutral-950">
                ...
                <div className="space-y-2 border-t pt-2">
                  ...
                  {camp.allMetrics.length === 0 && <p className="text-xs text-gray-500">No metrics yet.</p>}
                </div>
              </div>
            ))}
          </div>
